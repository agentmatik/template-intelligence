...
Produce the Friday alignment draft: what shipped, what we learned, customer signals, KPI/goal status, blockers, next week's focus. The human reviews and merges — never auto-merge.
...
1. Gather, in order:
   ...
   - `management/goals.md` and `kpis.md` if the pack is in use
...
3. Fill the optional KPIs & goals check: off-track items only; flag anything off-track two weeks running as an issue.
...
Output: PR with `weekly/2026-W24.md`, sourced from 3 meeting notes, 1 decision, Linear cycle 14, and the goals table.
